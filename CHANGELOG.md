...
- Inject `cls` (not `self`) as the first parameter of `#[classmethod]` stubs, matching typeshed conventions. Python classmethods bind the class as `cls`, so the previous `def name(self, ...)` was incorrect; when the Rust method also declared an explicit `cls: &Bound<'_, PyType>` first parameter it leaked into the stub (e.g. `def create(self, cls: type, x: int)`). `#[classmethod]`s now emit `def name(cls, ...)` with a bare `cls`, and the PyO3-injected `cls` receiver is excluded from the parameter list. Instance methods (`self`) and `#[staticmethod]`s are unchanged. (#9)
...
- CI now tests on `ubuntu-24.04-arm` to cover Linux ARM64 (glibc).
