...
| `Option<T>` | `T \| None` (py ≥ 3.10) or `Optional[T]` |
| `Vec<T>` | `list[T]` |
| `HashMap<K, V>` / `BTreeMap<K, V>` | `dict[K, V]` |
| `(A, B, ...)` tuple | `tuple[A, B, ...]` |
| `&Bound<PyAny>` / `PyObject` / `impl IntoPyObject` | `Any` + emit warning |
| unknown / unresolvable cross-crate type | `Any` + emit warning |
...
[output]
# Affects Optional[T] vs T | None syntax
python_version = "3.10"
add_header = true   # prepend "# Auto-generated by Rylai"

[[override]]
# Manually provide a stub line for items that cannot be inferred
