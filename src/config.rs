...
pub struct Config {
    ...
    /// Per-file `__all__` customisation: include/exclude names and override the private filter.
    #[serde(default, rename = "all")]
    pub all: Vec<AllEntry>,
...
pub struct OutputConfig {
    ...
    /// Global default: exclude names that start with `_` (private / dunder) from `__all__`.
    /// Can be overridden per file via `[[all]]`.
    #[serde(default)]
    pub all_include_private: bool,
}
...
///
/// `exclude` has the highest priority; `include` overrides the private filter for names that were
/// actually emitted; `include_private` overrides the global [`OutputConfig::all_include_private`]
/// for this file only.
#[derive(Debug, Deserialize)]
pub struct AllEntry {
    /// Relative path using `/`, e.g. `abcd.pyi` or `pkg/sub.pyi` (must match output layout).
    pub file: String,
    /// Override the global `all_include_private` for this file only.
    pub include_private: Option<bool>,
    /// Top-level names already emitted in this stub to keep in `__all__` even when the `_` filter
    /// would drop them (cannot add symbols that were not generated).
    #[serde(default)]
    pub include: Vec<String>,
    /// Names that are always excluded from `__all__` (highest priority).
    #[serde(default)]
    pub exclude: Vec<String>,
}

/// Resolved per-file `__all__` configuration after merging global defaults with all matching
/// `[[all]]` entries for a given stub file.
pub struct ResolvedAllConfig {
    pub include_private: bool,
    pub include: std::collections::HashSet<String>,
    pub exclude: std::collections::HashSet<String>,
}

/// Merge global `[output]` defaults with every `[[all]]` entry whose `file` matches `rel_path`.
///
/// Multiple matching entries are merged: `include`/`exclude` sets are unioned; the **last**
/// matching entry's `include_private` (if set) wins.
pub fn resolve_all_config(config: &Config, rel_path: &Path) -> anyhow::Result<ResolvedAllConfig> {
    let key = canonical_stub_rel_path(rel_path);
    let mut include_private = config.output.all_include_private;
    let mut include = std::collections::HashSet::new();
    let mut exclude = std::collections::HashSet::new();

    for e in &config.all {
        let p = normalize_add_content_file(&e.file)
            .map_err(|err| anyhow::anyhow!("[[all]].file {:?}: {}", e.file, err))?;
        if canonical_stub_rel_path(&p) != key {
            continue;
        }
        if let Some(ip) = e.include_private {
            include_private = ip;
        }
        for name in &e.include {
            include.insert(name.clone());
        }
        for name in &e.exclude {
            exclude.insert(name.clone());
        }
    }

    Ok(ResolvedAllConfig {
        include_private,
        include,
        exclude,
    })
